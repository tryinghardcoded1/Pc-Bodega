...
function AdminDashboard({ onOpenChat }: { onOpenChat: (id: string) => void }) {
  ...
  const [activeTab, setActiveTab] = useState<'orders' | 'messages' | 'users'>('orders');
  ...
  useEffect(() => {
    ...
    const qUsers = query(collection(db, 'users'), orderBy('updatedAt', 'desc'), limit(100));
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
       <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-8 gap-4">
          ...
          <div className="flex bg-white/5 p-1 rounded-xl border border-white/5">
             ...
             <button 
               ...
               className={`px-6 py-2 rounded-lg text-xs font-bold transition-all uppercase tracking-widest ${activeTab === 'users' ? 'bg-sky-500 text-black' : 'text-slate-400 hover:text-white'}`}
             >
                Users {users.length > 0 && <span className="ml-2 bg-black/20 px-1.5 rounded-full">{users.length}</span>}
             </button>
          </div>
       </div>
       ...
       <div className="grid grid-cols-1 gap-6">
          {loading ? (
             ...
             users.length === 0 ? (
                <div className="text-center py-20 bg-bg-card border border-white/5 rounded-3xl">
                   <p className="text-slate-500 font-medium">No registered users found.</p>
                </div>
             ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                   {users.map((u) => (
                      <div key={u.id} className="bg-bg-card border border-white/5 p-6 rounded-2xl shadow-xl hover:border-white/10 transition-all">
                         <div className="flex items-center gap-4 mb-4">
                            <div className="w-12 h-12 rounded-xl bg-sky-500/10 flex items-center justify-center font-bold text-sky-400 text-lg">
                               {u.displayName?.charAt(0) || u.email?.charAt(0) || '?'}
                            </div>
                            <div className="flex-1 min-w-0">
                               <h4 className="font-bold text-sm tracking-tight truncate">{u.displayName || 'No Name'}</h4>
                               <p className="text-[10px] text-slate-500 truncate">{u.email}</p>
                            </div>
                         </div>
                         <div className="space-y-2 mb-6">
                            <div className="flex items-center gap-2 text-[10px]">
                               <span className="text-slate-600 font-bold uppercase truncate">Phone:</span>
                               <span className="text-slate-400 truncate">{u.phone || 'N/A'}</span>
                            </div>
                            <div className="flex items-start gap-2 text-[10px]">
                               <span className="text-slate-600 font-bold uppercase">Address:</span>
                               <span className="text-slate-400 line-clamp-2 text-left">{u.address || 'N/A'}</span>
                            </div>
                         </div>
                         <div className="flex gap-2">
                            <button 
                              onClick={() => {
                                 const c = chats.find(ch => ch.id === u.id);
                                 if (c) onOpenChat(c.id);
                                 else {
                                   alert('No active chat thread found for this user.');
                                 }
                              }}
                              className="flex-1 py-2 bg-white/5 rounded-lg text-[10px] font-bold uppercase hover:bg-sky-500 transition-all hover:text-black flex items-center justify-center gap-2"
                            >
                               <MessageSquare className="w-3 h-3" /> Chat
                            </button>
                            <button 
                              onClick={async () => {
                                if (!window.confirm(`Are you sure you want to delete user data for ${u.displayName}? This will NOT delete their Auth account.`)) return;
                                try {
                              ...
                              className="px-3 py-2 bg-white/5 rounded-lg text-red-500 hover:bg-red-500 hover:text-white transition-all flex items-center justify-center"
                            >
       ...
       <div className="flex-1 bg-bg-card border border-white/10 rounded-2xl sm:rounded-3xl overflow-hidden flex flex-col shadow-2xl">
          ...
          <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-4 custom-scrollbar relative">
             ...
             {messages.map((m) => {
               ...
               const isAdminEditingUser = isUserAdmin && !isOwnMessage;
               // Allow any user to edit their own text messages, or admin to edit anything
               const canEdit = (isOwnMessage || isUserAdmin) && m.text; 
               ...
               return (
                <div key={m.id} className={`flex ${m.senderId === 'system' ? 'justify-center w-full my-4' : (m.senderId === auth.currentUser?.uid ? 'justify-end' : 'justify-start')} group`}>
                   <div className={`
                     ...
                     text-xs sm:text-sm relative overflow-hidden group/msg min-w-[60px]
                   `}>
                      {m.imageUrl && (
                        <img 
                          src={m.imageUrl} 
                        ...
                        editingMessageId === m.id ? (
                          <div className="flex flex-col gap-2 min-w-[200px] py-1">
                            <textarea 
                              ...
                              className={`w-full bg-black/10 border border-black/20 rounded-lg p-2 text-xs sm:text-sm outline-none ${isOwnMessage ? 'text-black placeholder-black/50' : 'text-white placeholder-white/50'}`}
                              rows={3}
                              autoFocus
                            />
                            <div className="flex justify-end gap-2">
                              <button onClick={() => setEditingMessageId(null)} className="flex items-center gap-1 px-2 py-1 bg-black/5 hover:bg-black/10 rounded-md transition-colors text-[10px] font-bold uppercase">
                                <X className="w-3 h-3" /> Cancel
                              </button>
                              <button onClick={() => saveEdit(m.id)} className="flex items-center gap-1 px-2 py-1 bg-black/20 hover:bg-black/30 rounded-md transition-colors text-[10px] font-bold uppercase">
                          ...
                          <div className="markdown-body relative">
                            ...
                            {m.isEdited && <span className={`text-[8px] opacity-40 block mt-1 italic ${isOwnMessage ? 'text-black' : 'text-white'}`}>(edited)</span>}
                            {canEdit && (
                              <button 
                                onClick={() => {
                                ...
                                className={`p-1 opacity-0 group-hover/msg:opacity-100 transition-opacity hover:bg-black/10 rounded absolute top-0 right-0 ${isAdminEditingUser ? 'text-red-500' : (isOwnMessage ? 'text-black/40' : 'text-white/40')}`}
                                title={isAdminEditingUser ? "Admin: Edit User Message" : "Edit message"}
                              >
